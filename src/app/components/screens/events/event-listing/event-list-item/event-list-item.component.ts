...
import { IFirebaseUserObject, IFirebaseTicketObject } from 'src/app/shared/models/Common/common.model';
...
export class EventListItemComponent implements OnInit {
  ...
  constructor(private dialogService: NbDialogService,
    ...
    if (user.events) {
      if (user.events.CBM2) {
        ...
        this.utilService.GetTicketStatusFromFirebase(user.events.GAINIGHT.ticketID);
      } else if (user.events.bumbastic) {
        this.utilService.GetTicketStatusFromFirebase(user.events.bumbastic.ticketID);
      }
    }
  ...
  async ngOnInit() {
    ...
    if (user.events) {
      if (user.events.CBM2) {
        const ticket = await this.utilService.GetTicketStatusFromFirebase(user.events.CBM2.ticketID);
  ...
  async viewTicket() {
    ...
    try {
      ...
      const secret = this.registrationService.decyrptSecret(user.encryptedSecret, sha256(this.password));
      ...
      if (xdr == null) {
        this.utilService.showErrorToast('Unable to get ticket', 'Ooops something went wrong!')
        this.showSpinner = false;
        return null;
      }
      ...
      const ticketID = user.events.CBM2.ticketID;
      const qrcodeObject = { xdr: xdr, emailHash: this.registrationService.hashEmail(user.email.toLowerCase()), ticketID: ticketID };
  ...
  async getTicket() {
    ...
    try {
      ...
      if (!ticketObject) {
        ...
        this.utilService.showErrorToast('Unable to get ticket', 'Failed to get ticket!!!');
        return;
      }
      ...
      await this.utilService.GetTicketStatusFromFirebase(user.events.CBM2.ticketID);
      ...
      this.utilService.showErrorToast('Ooops something went wrong', 'Failed to get ticket!')
      this.showSpinner = false;
